...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3 mb-0">Message from {{ $message->name }}</h1>
        ...
        @if($message->admin_reply)
            <div class="card mb-4 border-success">
                ...
                <div class="card-body">
                    ...
                    <small class="text-muted mt-2 d-block">Replied at: {{ $message->replied_at?->format('M d, Y h:i A') }}</small>
                </div>
            </div>
        @endif
    </div>
                ...
                <form action="{{ route('admin.contact-messages.update', $message) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" id="status" name="status">
                            ...
                            <option value="replied" {{ old('status', $message->status) == 'replied' ? 'selected' : '' }}>Replied</option>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('admin_reply') is-invalid @enderror" id="admin_reply" name="admin_reply" rows="6">{{ old('admin_reply', $message->admin_reply) }}</textarea>
                        @error('admin_reply')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
